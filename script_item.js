...
const getItemsFromDB = async () => {
  ...
  await fetch(`http://${backendIPAddress}/items`, options)
    ...
    .then((data) => {
      itemsData = data;
      itemsData.sort((a, b) => parseInt(a.created_date - b.created_date));
    })
    .catch((error) => console.error(error));
};
...
const addItem = async () => {
  const _item = document.getElementById("item-to-add").value;
  const _name = document.getElementById("name-to-add").value;
  const _price = document.getElementById("price-to-add").value;
  const table_body = document.getElementById("main-table-body");
    ...
    .then((tmp) => {
      table_body.innerHTML += `
        <tr id="${data.item_id}">
            ...
            <td>${data.name}</td>
            <td>${data.price}</td>
            <td><button class="delete-row" onclick="deleteItem('${data.item_id}')">ลบ</button></td>
        </tr>
        `;
    })
    .catch((error) => console.error(error));
